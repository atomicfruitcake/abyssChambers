...
static
const char * matchSpaces(const char * src) {
    return *src == '\0' || isspace( * src) ? skipSpaces(src) : NULL;
}

static
const char * matchTerminal(const char * src, char terminal) {
    return terminal == ' ' ? matchSpaces(src) :
        tolower( * src) == tolower(terminal) ? src + 1 : NULL;
}

static
const char * matchTag(const char * src,
    const char * tag) {
...
static
const char * matchParam(const char * src, PARAM * par, int loose) {
    ...
    forEachObject(obj) {
        ...
        for (tag = obj -> tags;* tag != NULL; tag++) {
            ...
            if (behindMatch != NULL &&
                compareWithParam( * tag, distance, par) > 0 &&
                (!loose || * skipSpaces(behindMatch) == '\0')) {
                par -> tag = * tag;
                par -> obj = obj;
...
int matchCommand(const char * src,
    const char * pattern) {
    PARAM * par;
    ...
    for (src = skipSpaces(src); src != NULL && * pattern != '\0'; pattern++) {
        src = isupper( * pattern) ? matchParam(src, paramByLetter( * pattern),
                pattern[1] == '?') :
            *
            pattern == '?' ? src : matchTerminal(src, * pattern);
    }
